...
class DownloadViewHelper(
    ...
    init {
        ...
        buttonOpenDownload.setOnClickListener {
            if (download.openIntent != null) {
                try {
                    App.instance.startActivity(download.openIntent)
        ...
        when {
            download.isDownloadRunning -> showRunningState()
    ...
    private fun deleteFile() {
        if (download.delete(activity)) {
            showStartState()
        }
    }

    private fun startDownload() {
        if (download.start(activity)) {
            showRunningState()
        }
    }
    ...
    private fun registerDownloadStateObservers() {
        if (download.isDownloadable) {
            App.instance.state.download.of(download.identifier)
                .observe(activity) {
                    when (it) {
                        DownloadStateLiveData.DownloadStateCode.COMPLETED -> {
                            if (!download.isDownloadRunning && download.downloadExists) {
                                showEndState()
                            }
                        }
